...
Mov* GenerateMoves(int& moveLen,int player)
{
	...
	while(p != boardk)
	{
		...
		{
			for(int i = ( p->x - 3 < 0 ? 0 : p->x - 3 );i <= ( p->x + 3 > width ? width : p->x + 3); i++)
			{
				for (int j = ( p->y - 3 <0 ? 0: p->y -3); j <= (p->y + 3 > height ? height:p->y +3); j++)
				{
	...
	{
		srand(GetTickCount());
		result[count].x = rand()%width;
		result[count].y = rand()%height;
		result[count].val = 0;
		count++;
	}
